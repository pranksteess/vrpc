...
require_once(dirname(__FILE__). '/' . 'type/pb_fixed32.php');
require_once(dirname(__FILE__). '/' . 'type/pb_fixed64.php');
    ...
    private function _ParseFromArray($length=99999999)
    {
        ...
        while ($this->reader->get_pointer() - $_begin < $length)
        {
            ...
            if (!isset($this->fields[$messtypes['field']]))
            {
				...
				{
                    $consume = new PB32Bit($this->reader);                
				}  
                else if ($messtypes['wired'] == PBMessage::WIRED_64BIT)
                {
                    $consume = new PB64Bit($this->reader);
                }
                else
                {
		...
		//var_dump($this->values[$messtypes['field']]);
            }
        }
    }
