...
static const int MONITOR_INTERVAL = 10;
...
void MonitorWorker()
{
    ...
    fprintf( stderr, "MonitorWorker init ok\n" );
    fprintf( stderr, "Tnm2Attr: Req[%d] Err[%d] AvgTime[%d] ErrRatio[%d]\n",
             sAttr.iReqCountAttr, sAttr.iErrCountAttr, sAttr.iUsedTimeAttr,
             sAttr.iErrRatioAttr );
...
extern "C" int spp_handle_process(unsigned flow, void* arg1, void* arg2)
{
    ...
    uint32_t uiUsedTime = 0;
    clib_timer timer;
    char process_name[] = "process";
...
*/
   ...
   {
       uiUsedTime = timer.end_get_ms(process_name);
       g_cRecoder.Recode( false, uiUsedTime );
   ...
   switch( cHead.cmd() )
   {
		   case vrpc::example::QUERYINFO_CMD:
				   iRet = g_cService.QueryInfo( cReq1, cResp1 );
				   pcResp = &cResp1;
				   break;
		   case vrpc::example::UPDATEINFO_CMD:
				   iRet = g_cService.UpdateInfo( cReq2, cResp2 );
				   pcResp = &cResp2;
				   break;
		   default:
		       uiUsedTime = timer.end_get_ms(process_name);
   ...
   uiUsedTime = timer.end_get_ms(process_name);
   g_cRecoder.Recode( true, uiUsedTime );
